...
conn = sqlite3.connect("D:\Grocery using Tkinter\Database\grocery.db")
c = conn.cursor()

# date
date = datetime.datetime.now().date()

# temprory lists like sessions
products_list = []
product_price = []
product_quantity = []
product_id = []
...
class Application:
    def __init__(self, master):
        ...
        # components
        self.heading = Label(self.left, text="Grocery Store", font=('arial 40 bold'), bg='white')
        ...
        # button
        self.search_btn = Button(self.left, text="Search", width=22, height=2, bg='orange', command=self.search)
        self.search_btn.place(x=350, y=120)

        # fill it later by the function search
    ...
    def change_func(self):
        # get the amount given by the customerand the amount generated by the computer
    ...
    def generate_bill(self):
        ...
        directory = "D:/Grocery using Tkinter/Invoice/" + str(date) + "/"
        ...
        for t in products_list:
            f.write(
                "\n\t\t\t" + str(r) + "\t" + str(products_list[i] + "------") + str(product_quantity[i]) + "\t\t" + str(
                    product_price[i]))
            i += 1
...
result = c.execute('SELECT "id","name" FROM "inventory"')
rows = c.fetchall()
for row in rows:
